...
- (IBAction)enviar:(id)sender {
    ...
    UIAlertView* mes=[[UIAlertView alloc] initWithTitle:@"Mi decisión" message:@"Entiendo que Jesús tiene una iglesia donde está juntando a todos sus hijos. Deseo formar parte de esa iglesia y prepararme para encontrarme con Jesús cuando él vuelva en gloria y majestad." delegate:self cancelButtonTitle:@"Acepto" otherButtonTitles: nil];
    [mes show];
    ...
    if(p4a.isOn){
        ...
        [post appendString:@"&numeroLeccion=17&idRegistro="];
        [post appendString:idRegistro];
        [post appendString:@"&respuestas="];
        [post appendString:_respuesta];        NSData *postData = [post dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:YES];
